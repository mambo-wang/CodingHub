...
```json
{
  "platform": "cas|workspace|uis|onestor（可选）",
  "resource_name": "资源名称（可选，模糊匹配）",
  "ip_address": "IP 地址（可选）",
  "limit": 100,
  "offset": 0
}
...
```
用户：列出所有 CAS 资源
Skill：调用 showtime_list_resources 工具（platform="cas"）

用户：查找 IP 为 192.168.1.100 的资源
Skill：调用 showtime_list_resources 工具（ip_address="192.168.1.100"）
```
...
```
用户：查看资源 res001 的详情
Skill：调用 showtime_get_resource_detail 工具
```
...
```
用户：列出所有指标类型
Skill：调用 showtime_list_metric_types 工具

用户：查看 CAS 平台的指标类型
Skill：调用 showtime_list_metric_types 工具（platform="cas"）
```
...
{
  "resource_id": "资源ID",
  "metric_type": "指标类型，如 cpu_usage、mem_usage",
  "hours": 24
}
```
...
Skill：调用 showtime_get_metric_trend 工具（resource_id="res001", metric_type="cpu_usage", hours=24）

用户：查看资源 res001 最近 7 天的内存趋势
Skill：调用 showtime_get_metric_trend 工具（resource_id="res001", metric_type="mem_usage", hours=168）
```
...
Skill：调用 showtime_get_metric_summary 工具
```
...
Skill：调用 showtime_list_latest_metrics 工具
```
...
Skill：调用 showtime_rag_chat 工具

用户：继续 sess001 会话，问"那如何关机？"
Skill：调用 showtime_rag_chat 工具（session_id="sess001", question="那如何关机？"）
```
...
Skill：调用 showtime_list_sessions 工具
```
...
```
用户：删除会话 sess001
Skill：调用 showtime_delete_session 工具
```
...
```
用户：列出所有知识库
Skill：调用 showtime_list_knowledge_bases 工具
```
...
```
用户：列出所有 UIS 资源
Skill：调用 showtime_list_resources 工具（platform="uis"）
```
...
Skill：调用 showtime_get_resource_detail 工具（resource_id="res001"）
```
...
Skill：调用 showtime_list_latest_metrics 工具（resource_id="res001"）
```
...
Skill：调用 showtime_rag_chat 工具（kb_id="kb001", question="CAS虚拟化平台支持哪些功能？"）
```
...
Skill：调用 showtime_rag_chat 工具（session_id="sess001", question="如何创建虚拟机？"）
```
...
Skill：调用 showtime_list_sessions 工具（kb_id="kb001"）

用户：删除会话 sess001
Skill：调用 showtime_delete_session 工具（session_id="sess001"）
```
