...
**Input**: Optionally specify a change name. Supports `--max-tasks N` (default 20) to control plan splitting.
...
4. **Determine plan splitting**
   ...
   If `--max-tasks` is less than 1, treat as default (20). Count total tasks. If total > `--max-tasks`:
