---
okf_version: "0.2"
aliases:
- 项目文档索引
- 文档索引
- 知识笔记索引
---

<!-- 自动生成于 2026-08-27T17:35:03+08:00 | Health Score: 0/100 | 本文件由系统自动维护 -->

# 项目文档索引

## 模块文档

* [概览与统计模块 (backend-overview)](modules/backend-overview.md) - title: 概览与统计模块 (backend-overview)
* [CodingHub 仓库总览](modules/codinghub.md) - CodingHub (ai-tool-square) 全栈 AI 工具导航平台的系统级架构与模块导航
* [MCP模块 (backend-mcp)](modules/backend-mcp.md) - title: MCP模块 (backend-mcp)
* [RAG服务 (rag)](modules/rag.md) - title: RAG服务 (rag)
* [业务组件 (frontend-components)](modules/frontend-components.md) - title: 业务组件 (frontend-components)
* [前端应用 (frontend)](modules/frontend.md) - title: 前端应用 (frontend)
* [反馈与通知模块 (backend-feedback)](modules/backend-feedback.md) - title: 反馈与通知模块 (backend-feedback)
* [后端服务 (backend)](modules/backend.md) - title: 后端服务 (backend)
* [基础设施层 (backend-infra)](modules/backend-infra.md) - title: 基础设施层 (backend-infra)
* [微课模块 (backend-video)](modules/backend-video.md) - title: 微课模块 (backend-video)
* [服务层 (frontend-services)](modules/frontend-services.md) - title: 服务层 (frontend-services)
* [标签模块 (backend-tag)](modules/backend-tag.md) - title: 标签模块 (backend-tag)
* [核心模块 (backend-core)](modules/backend-core.md) - title: 核心模块 (backend-core)
* [状态与类型 (frontend-stores)](modules/frontend-stores.md) - title: 状态与类型 (frontend-stores)
* [知识库模块 (backend-kb)](modules/backend-kb.md) - title: 知识库模块 (backend-kb)
* [论坛模块 (backend-forum)](modules/backend-forum.md) - title: 论坛模块 (backend-forum)
* [页面与路由 (frontend-pages)](modules/frontend-pages.md) - title: 页面与路由 (frontend-pages)

## 知识笔记

* [CodeBuddy 插件清单入口是 .codebuddy-plugin/plugin.json，根目录 plugin.json 不能替代](../notes/2026-08-27-codebuddy-插件清单入口是-codebuddy-pluginpluginjson根目录-pluginjson-不.md) - pitfall (pitfall, 2026-08-27)
* [Windows 文件锁导致 JGit bare 仓库重建失败，仓库目录应带版本号](../notes/2026-08-27-windows-文件锁导致-jgit-bare-仓库重建失败仓库目录应带版本号.md) - pitfall (pitfall, 2026-08-27)
* [命令注册名 = frontmatter name（无则文件名），点分文件名会产生错误命令名](../notes/2026-08-27-命令注册名-frontmatter-name无则文件名点分文件名会产生错误命令名.md) - pitfall (pitfall, 2026-08-27)
* [JPA @PreUpdate 会在任意 save() 时刷新 updatedAt，读改写式计数更新会污染更新时间](../notes/2026-08-26-jpa-preupdate-会在任意-save-时刷新-updatedat读改写式计数更新会污染更新时间.md) - pitfall (pitfall, 2026-08-26)
* [Spring Boot 3 应使用 jakarta.servlet 命名空间，勿误用 jgit 包下的 HttpServletRequest](../notes/2026-08-26-spring-boot-3-应使用-jakartaservlet-命名空间勿误用-jgit-包下的-httpservle.md) - pitfall (pitfall, 2026-08-26)
* [内容实体计数更新应改用 repository 层原子 SQL（@Modifying）而非读改写 + save()](../notes/2026-08-26-内容实体计数更新应改用-repository-层原子-sqlmodifying而非读改写-save.md) - decision (decision, 2026-08-26)
