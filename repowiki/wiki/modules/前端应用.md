...
| `knowledge.ts` | 知识库与 RAG | `getList()`、`create()`、`batchUpload()`、`search()`、`getConfig()`、`getDocumentStatus()` |
...
平台首页，展示工具列表，支持分类筛选、标签过滤、关键词搜索和排序。包含工具上传入口。

### DetailPage（工具详情）

展示单个工具的完整信息：Markdown 内容渲染、文件列表、评论区、点赞/收藏互动。支持管理员置顶。
...
- **KnowledgeDetailPage**：知识库详情：文档列表、上传、搜索、RAG 配置面板
...
- **knowledge/**：KnowledgeCard、DocumentList、DocumentUpload、KnowledgeSearch、ConfigPanel、StatusBadge
...
| `tool.ts` | `ToolDetailDTO`、`ToolSummary`（含互动统计字段） |
...
| `knowledge.ts` | `KnowledgeBase`、`RagDocument`、`RagDocumentStatus`、`DocumentStatus`（枚举）、`KbConfig`、`KbSearchResult`、`KbCreateRequest`、`KbSearchRequest`、`KbPageResponse` |
