...
关键方法：
- `getTools()` — 分页查询工具列表，支持按分类、关键词筛选，支持 hot/latest/name 三种排序
...
| score | BigDecimal | 热度分 = view×1 + like×3 + comment×5 |
| pinned | Boolean | 是否置顶 |
...
| 方法 | 路径 | 说明 | 权限 |
|------|------|------|------|
| GET | `/api/v1/tools` | 分页查询工具列表（支持 categoryId、keyword、sortBy） | 公开 |
