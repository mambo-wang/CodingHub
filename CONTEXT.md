...
**帖子（Forum Post）**:
论坛中的一篇内容，含标题、正文、分类、标签与可见性（公开/私有），带浏览量、点赞、评论等互动数据。
_Avoid_: 文章、文档、主题

**正文格式（Content Format）**:
帖子正文在库中的表示形式，取值 MARKDOWN 或 HTML。是**存储格式**而非来源格式——HTML 帖子的原文就是 HTML，可无损再编辑。
_Avoid_: 内容类型、渲染格式、来源格式

**导入（Import）**:
上传 `.md`/`.html` 文件，服务端解析为帖子正文后**即丢弃文件**，产物只有帖子本身。
_Avoid_: 上传、附件

**上传（Upload）**:
文件本身落盘留存，成为可被单独引用的附件或文档（工具附件、知识库文档、帖子插图）。
_Avoid_: 导入
