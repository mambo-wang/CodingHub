...
| 业务域 | 包路径 | 核心实体 |
|--------|--------|----------|
| 工具市场 | `model/` | Tool, Category, ToolFile |
| 用户系统 | `model/` | User |
| 统一互动 | `model/` | UnifiedComment, UnifiedLike, UnifiedFavorite |
| 实时聊天 | `model/` | ChatMessage, ChatReaction |
| 视频模块 | `model/video/` | Video, Danmaku |
| 论坛模块 | `model/forum/` | ForumPost, ForumCategory, ForumTag, ForumPostTag |
| 标签系统 | `model/tag/` | Tag, ToolTag, VideoTag |
| 通知系统 | `model/notification/` | Notification |
| 知识库 | `model/kb/` | KnowledgeBase |
| 反馈系统 | `model/feedback/` | FeedbackMessage |
...
| category | Category | FK, NOT NULL, LAZY | 所属分类 |
...
| uploader | User | FK, NOT NULL, LAZY | 上传者 |
| status | Status 枚举 | NOT NULL, 默认 NORMAL | 软删除标记 |
...
| role | Role 枚举 | NOT NULL, 默认 USER | 角色：USER / ADMIN / SUPER_ADMIN |
| status | AccountStatus 枚举 | NOT NULL, 默认 ACTIVE | 状态：ACTIVE / PENDING / REJECTED / DISABLED |
...
本域采用 **多态目标（Polymorphic Target）** 设计，通过 `targetType + targetId` 组合实现对 Tool、ForumPost、Video 三种内容类型的统一评论/点赞/收藏，避免为每种内容类型创建独立的互动表。
...
| status | VideoStatus 枚举 | NOT NULL, 默认 NORMAL | 状态 |
...
| user | User | FK, NOT NULL, LAZY | 发送者 |
...
| viewCount / likeCount / commentCount | Integer | 默认 0 | 互动计数 |
| status | ForumPostStatus 枚举 | NOT NULL, 默认 NORMAL | 状态：NORMAL / DELETED / HIDDEN |
| visibility | ForumPostVisibility 枚举 | NOT NULL, 默认 PUBLIC | 可见性：PUBLIC / PRIVATE |
| score | BigDecimal(10,2) | 默认 0 | 热度分 |
| pinned | Boolean | NOT NULL, 默认 false | 是否置顶 |
| createdAt / updatedAt | LocalDateTime | - | 时间戳 |
...
| tagType | TagType 枚举 | NOT NULL | 类型：TOOL / FORUM / VIDEO |
...
|------|------|------|------|
| id | Long | PK, 自增 | 主键 |
| user | User | FK, NOT NULL, LAZY | 接收者 |
| type | NotificationType 枚举 | NOT NULL | 类型：COMMENT_REPLY / LIKE / ADMIN_APPROVED / ADMIN_REJECTED |
...
| status | KbStatus 枚举 | NOT NULL, 默认 NORMAL | 状态 |
| createdAt / updatedAt | LocalDateTime | - | 时间戳 |
...
| category | FeedbackCategory 枚举 | NOT NULL, 默认 SUGGESTION | 分类：SUGGESTION / BUG_REPORT / PRAISE / OTHER |
| user | User | FK, LAZY | 提交用户（可空） |
| ipHash | String(64) | 可空 | IP 哈希 |
| status | Status 枚举 | NOT NULL, 默认 NORMAL | 状态 |
| adminReply | TEXT | 可空 | 管理员回复 |
| repliedBy | User | FK, LAZY | 回复管理员 |
...
| 实体 | 状态枚举 | 值 |
|------|----------|-----|
| Tool | Tool.Status | NORMAL, DELETED |
| Video | VideoStatus | NORMAL, DELETED |
| ForumPost | ForumPostStatus | NORMAL, DELETED, HIDDEN |
| KnowledgeBase | KbStatus | NORMAL, DELETED |
| FeedbackMessage | FeedbackMessage.Status | NORMAL, DELETED |
| ChatMessage | String status | "ACTIVE", "DELETED" |
| User | AccountStatus | ACTIVE, PENDING, REJECTED, DISABLED |
...
- `updatedAt`：每次更新自动刷新
- 仅创建型实体（如 Notification、Danmaku）只有 `createdAt`

### 3. 关联策略

| 策略 | 使用场景 | 示例 |
|------|----------|------|
| `@ManyToOne(LAZY)` + `@JoinColumn` | 强关联、需 JOIN 查询 | Tool -> Category, Tool -> User, Notification -> User |
| 冗余 ID 字段（无 FK） | 弱关联、多态目标 | UnifiedComment.userId, ForumPost.authorId, Video.uploaderId |
| `@IdClass` 复合主键 | 多对多关联表 | ToolTag(toolId, tagId), VideoTag(videoId, tagId), ForumPostTag(postId, tagId) |

**设计原则**：
- 需要 `JOIN FETCH` 预加载的场景使用 `@ManyToOne` 关联（如 Tool 详情页需同时加载 Category 和 Uploader）
- 仅需 ID 引用的场景使用冗余字段（如 UnifiedComment 只需 userId 做查询条件，无需加载完整 User 对象）
- 多态互动表（UnifiedLike/Comment/Favorite）通过 `targetType + targetId` 字符串组合实现，避免外键约束限制多态扩展

### 4. 热度分与计数器冗余

Tool、Video、ForumPost 三个内容实体均维护 **冗余计数器**（viewCount, likeCount, commentCount）和 **综合热度分**（score），通过实体方法（如 `incrementViewCount()`）在业务操作时同步更新，避免列表查询时的 COUNT 聚合开销。
...
- **[backend-service](backend-service.md)**：Service 层调用本模块的 Repository 接口完成业务逻辑，包括 ToolService、UserService、CommentService、LikeService、FavoriteService、VideoService、ForumService、NotificationService 等
